...
- **文字コード**：UTF-8。
- **コメントの書き分け**：HTMLコメント（`<!-- -->`）はブラウザへそのまま配信されるため、テンプレート（`.php`）内の説明・設計意図には使わず、PHPコメント（`<?php /* ... */ ?>`）で書く。CSSのコメントはソースに残してよい（本番配信物からは `tools/deploy-prep.mjs` が除去する。README「本番デプロイ準備」参照）。
